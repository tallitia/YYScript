...
namespace RoyT.AStar
{
    ...
    public sealed class Grid
    {
        ...
        public Grid(int dimX, int dimY, float defaultCost = 1.0f)
        {
            if (defaultCost < 1)
            {
                throw new ArgumentOutOfRangeException(
                    "Argument with value " + defaultCost + " is invalid. The cost of traversing a cell cannot be less than one");
            }
        ...
        public void SetCellCost(Position position, float cost)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(
                    "Argument with value " + cost + " is invalid. The cost of traversing a cell cannot be less than one");
            }

            if (position.X < 0 || position.X >= this.DimX)
            {
                UnityEngine.Debug.Log(string.Format("The x-coordinate {0} is outside of the expected range [0...{1}])", position.X, this.DimX));
                return ;
            }

            if (position.Y < 0 || position.Y >= this.DimY)
            {
                UnityEngine.Debug.Log(string.Format("The y-coordinate {0} is outside of the expected range [0...{1}])", position.Y, this.DimY));
                return ;
            }
        ...
        {
            return this.GetPath(start, end, MovementPatterns.Full);
        }
        ...
        {
            if (this.Weights[GetIndex(end.X, end.Y)] == float.PositiveInfinity)
            {
        ...
        private int GetIndex(int x, int y)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(
                    string.Format("The x-coordinate {0} is outside of the expected range [0...{1}])", x, this.DimX));
            }

            if (y < 0 || y >= this.DimY)
            {
                throw new ArgumentOutOfRangeException(
                    string.Format("The y-coordinate {0} is outside of the expected range [0...{1}])", y, this.DimY));
            }
